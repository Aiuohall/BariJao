...
CREATE TABLE IF NOT EXISTS listings (
    ...
    asking_price DECIMAL NOT NULL,
    ticket_image TEXT,
    status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'available', 'sold')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE POLICY "Allow users to see own listings" ON listings FOR SELECT USING (auth.uid() = user_id OR true); -- Simplified for prototype, in real app use auth.uid()

DROP POLICY IF EXISTS "Allow users to create listings" ON listings;
