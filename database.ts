...
const initSqlite = () => {
  ...
  try {
    ...
    // Use a dedicated SQLite schema. The Postgres schema (supabase.sql) uses
    // syntax SQLite cannot parse (gen_random_uuid(), TIMESTAMP WITH TIME ZONE,
    // RLS/policies), so we ship a SQLite-native schema instead.
